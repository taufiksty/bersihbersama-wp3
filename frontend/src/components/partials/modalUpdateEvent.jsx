...
export default function ModalUpdateEvent(props) {
	const [provinces, setProvinces] = React.useState([]);
	const [cities, setCities] = React.useState([]);
	const [districts, setDistricts] = React.useState([]);
	const [regionId, setRegionId] = React.useState({});
	const [imageFile, setImageFile] = React.useState(null);
	const [valueSubmit, setValueSubmit] = React.useState(props.event);
  ...
  const navigate = useNavigate();
	...
	const handleUpdateSubmit = (e) => {
		...
		axios
			.post(`http://localhost:8080/api/v1/events/update/${valueSubmit.id}`, formData, {
				headers: {
			...
			<div className="relative p-4 w-full max-w-2xl h-full md:h-auto">
				<div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                  ...
                  value={valueSubmit.date}
							...
							<div>
								<label
									htmlFor="total_people"
									className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
									Jumlah Minimum Partisipan
								</label>
								<input
									type="number"
									name="total_people"
									...
									placeholder="50"
                  required=""
                  value={valueSubmit.total_people}
									onChange={(e) =>
										setValueSubmit((oldState) => ({
											...oldState,
											total_people: e.target.value,
										}))
									}
								/>
							</div>
                  ...
                  value={valueSubmit.address}
							...
							<div>
								...
								<select
									...
									<option defaultValue={valueSubmit.province}>Pilih provinsi</option>
									{provinces.map((item) => (
										<option
											key={item.id}
							...
							<div>
								...
								<select
									...
									<option defaultValue={valueSubmit.city}>Pilih kota/kabupaten</option>
									{cities.map((item) => (
										<option
											key={item.id}
											value={toTitleCase(item.name)}
							...
							<div>
								...
								<select
									...
									<option defaultValue={valueSubmit.district}>Pilih kecamatan</option>
									{districts.map((item) => (
										<option
											key={item.id}
											value={toTitleCase(item.name)}>
                  ...
                  value={valueSubmit.link_map}
							...
							<div>
								...
								<input
									...
									id="link_groupwa"
									className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                  placeholder="https://chat.whatsapp.com/LxuUGPcxOf02AWoMmd38xc"
                  value={valueSubmit.link_groupwa}
									onChange={(e) =>
										setValueSubmit((oldState) => ({
											...oldState,
											link_groupwa: e.target.value,
										}))
									}
								/>
							</div>
